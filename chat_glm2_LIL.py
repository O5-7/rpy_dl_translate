...
class chat_glm2_LIL:
    def __init__(
        ...
        model_or_path: str,
        device: str = "auto",
        checkpoint_name: str = "checkpoint-3000",
    ):
        ...
        self.config = AutoConfig.from_pretrained(
            model_or_path, pre_seq_len=128, trust_remote_code=True
        )
        self.model = AutoModel.from_pretrained(
            model_or_path, trust_remote_code=True, config=self.config
        )

        prefix_state_dict = torch.load(
            os.path.join(model_or_path, "prefix", checkpoint_name, "pytorch_model.bin"),
    ...
    def translate(self, text: Union[str, List[str]], **kwargs):
        ...
        for input_text in text_iter:
            ...
            response, history = self.model.chat(
                self.tokenizer, "翻译这个LIL的台词:" + input_text, max_length=input_text_len
            )
...
if __name__ == "__main__":
    mt = chat_glm2_LIL(
        "../dl_models/chatglm2-6b-int4", checkpoint_name="checkpoint-3000"
    )
    test = [
